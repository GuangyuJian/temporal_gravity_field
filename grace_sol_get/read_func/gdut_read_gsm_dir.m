...
% this function dedicated to read all the grace-gsm file in a the same path
%----------------------------------------------------------------------------
% In   :
...
all_file=dir(fullfile(dir_in,'GSM*gfc'));%获取文件名

if length(all_file)~=0

else
    disp('尝试DDK')
    all_file=dir(fullfile(dir_in,'kfilter*gfc'));
end
...
if num_t==0
    show_time_tag;
    error("gdut_read_gsm_dir:    路径未找到文件");
else
    show_time_tag;
    disp("gdut_read_gsm_dir:    共检索到"+num_t+"个文件");
end
