function [sol_shc_vmds,sol_wk,info,shcvmds]=gdut_external_vmds_wk_r3(sol_shc_in,k_max,alpha,wk_max)
%
%----------------------------------------------------------------------------
% In   :
%
...
for tt=1:length(sf.time)
    ...
    loc=find(wk>=wk_max);
    loc=[loc+1];
    temp=sol_sf(ewhvmd,'ewh (mm)',myb.fir,myb.ceta);
    shcvmds(tt)=temp.sf2shc(myf,myb,'gc');
    noise_vmd(:,:,tt)=sum(ewhvmd(:,:,loc),3);
    sol_wk(:,tt)=wk;
    
end

sol_shc_in.change_type('gc');
sf.value=noise_vmd;
sol_shc_vmds=sol_shc_in-sf2shc(sf,myf,myb,'gc');
