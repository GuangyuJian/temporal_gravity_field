...
function[ou]=fit_harmonic_ts_4months(t,tws,t2)
% input:t应该为列向量
%           tws为列向量
if nargin<=3
    t1=[];
    ...
    X1=[ones(size(t1,1),1) t1 cos(2*t1*pi) sin(2*t1*pi)  cos(4*t1*pi)  sin(4*t1*pi) cos(6*t1*pi)  sin(6*t1*pi)     ];
    X2=[ones(size(t2,1),1) t2 cos(2*t2*pi) sin(2*t2*pi)  cos(4*t2*pi)  sin(4*t2*pi) cos(6*t2*pi)  sin(6*t2*pi)     ];
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               0
%   [b1,bint,~,~,~,] = regress(tws_temp,X1);
    ...
    Amp3=Root_Sum2(b1(7),b1(8));
    Amp3_std=[];
    Pha3=get_phase(b1(7),b1(8),Amp3);
    Pha3_std=[];

    ma1=2*sqrt((b1(3)/Amp1)^2*Dbb(3,3)+ (b1(4)/Amp1)^2*Dbb(4,4));
    ma2=2*sqrt((b1(5)/Amp2)^2*Dbb(5,5)+ (b1(6)/Amp2)^2*Dbb(6,6));
    ma3=2*sqrt((b1(7)/Amp3)^2*Dbb(7,7)+ (b1(8)/Amp3)^2*Dbb(8,8));
...
%     
    ...
    mp3=2*sqrt(Dbb(7,7) * (b1(8)/Amp3^2)^2+ Dbb(8,8) * (b1(7)/Amp3^2)^2);
    ...
    info=[Amp1 ma1;Pha1 mp1;Amp2 ma2;Pha2 mp2;Amp3 ma3;Pha3 mp3];
    Trend_std=Dbb(2,2);
    %%
